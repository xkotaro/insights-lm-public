/*
  # InsightsLM Application Database Schema - Complete Migration

  ## Overview
  This migration creates the complete database schema for the InsightsLM application,
  a knowledge management system that allows users to create notebooks, add sources,
  generate notes, and interact with content through AI-powered chat.

  ## New Tables

  ### 1. `profiles`
  User profile information synchronized with auth.users
  ...
  ### 2. `notebooks`
  Central organization unit for user content
  ...
  - `generation_status` (text) - Content generation status
  - `audio_overview_generation_status` (text) - Audio generation status
  - `audio_overview_url` (text) - Generated audio URL
  - `audio_url_expires_at` (timestamptz) - Audio URL expiration
  - `example_questions` (text[]) - Suggested questions
  - `created_at`, `updated_at` (timestamptz) - Timestamps

  ### 3. `sources`
  Content sources added to notebooks
  - `id` (uuid, primary key) - Unique identifier
  ...
  ### 4. `notes`
  User-created or AI-generated notes
  - `id` (uuid, primary key) - Unique identifier
  - `notebook_id` (uuid) - References notebooks
  - `title` (text) - Note title
  - `content` (text) - Note content
  - `source_type` (text) - Origin of note (user/ai)
  - `extracted_text` (text) - Source text if extracted
  - `created_at`, `updated_at` (timestamptz) - Timestamps
  ...
  - `session_id` (uuid) - Notebook ID (session identifier)
  - `message` (jsonb) - Chat message data

  ## Custom Types

  ### `source_type` ENUM
  Valid source types: 'pdf', 'text', 'website', 'youtube', 'audio'
  ...
  ### 3. `is_notebook_owner(notebook_id_param)`
  Helper function to check if current user owns a notebook

  ### 4. `is_notebook_owner_for_document(doc_metadata)`
  Helper function to check notebook ownership via document metadata

  ### 5. `match_documents(query_embedding, match_count, filter)`
  Vector similarity search function for semantic document retrieval

  ## Indexes

  Performance indexes created for:
  - Notebooks by user_id and updated_at
  - Sources by notebook_id, type, and processing_status
  - Notes by notebook_id
  - Chat histories by session_id
  - Vector similarity search on documents.embedding (HNSW index)
  ...
  ### Notebooks
  - Users can view, create, update, and delete only their own notebooks

  ### Sources
  - Users can manage sources only in notebooks they own

  ### Notes
  - Users can manage notes only in notebooks they own

  ### Documents
  - Users can manage documents only for notebooks they own
  - Ownership verified via metadata.notebook_id

  ### Chat Histories
  - Users can view, create, and delete chat histories only for their notebooks
  - session_id corresponds to notebook_id

  ## Storage Buckets

  ### 1. `sources` (Private)
  User-uploaded files (PDFs, documents, audio)
  - Max size: 50MB
  - Allowed types: PDF, text, DOC, audio formats
  - RLS: Users can only access files in their own notebooks

  ### 2. `audio` (Private)
  AI-generated audio content
  - Max size: 100MB
  - Allowed types: Audio formats (MP3, WAV, M4A, MP4)
  - RLS: Users can view/delete their own audio; service role can manage all

  ### 3. `public-images` (Public)
  Public image assets
  - Max size: 10MB
  - Allowed types: Image formats (JPEG, PNG, GIF, WEBP, SVG)
  - RLS: Public read access; service role can manage
  ...
  All configured with REPLICA IDENTITY FULL for complete change tracking.
  ...
  1. **Data Integrity**: All foreign keys have CASCADE DELETE to maintain referential integrity
  2. **Security First**: RLS is enforced on all tables with restrictive policies
  3. **Performance**: Strategic indexes for common query patterns
  4. **Scalability**: Vector search optimized with HNSW index
  5. **Automation**: Triggers handle profile creation and timestamp updates
  6. **File Organization**: Storage uses notebook_id as folder structure
*/
